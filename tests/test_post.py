async def test_create_post_unauthenticated(client) :
    response = await client.post("/auth/posts", json={"comment": "No token"})
    assert response.status_code == 401

async def test_new_post(client) :
    ...
    response = await client.post("/auth/posts", json={"comment" : "NEW COMMENT"}, headers=headers)
    assert response.status_code == 200
    assert response.json()["comment"] == "NEW COMMENT"
    
async def test_get_user_post_successful(client) :
    email = "test@gmail.com"
    password = "test1234"
...
async def test_get_user_post_unsuccessful(client) :
    ...
    response = await client.post("/auth/register", json={"email" : email2, "password" : password2}, headers={"x-idempotency-key": "test-key-123450"})
    assert response.status_code == 200
        
    response = await client.post("/auth/login", json={"email" : email2, "password" : password2})
    assert response.status_code == 200
